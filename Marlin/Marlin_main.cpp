...
static bool drain_injected_commands_P() {
  if (injected_commands_P != NULL) {
    ...
    cmd[i] = '\0';
    if (enqueue_command(cmd))     // success?
...
bool enqueue_command(const char* cmd, bool say_ok=false, bool echo=false) {
...
bool enqueue_and_echo_command(const char* cmd, bool say_ok/*=false*/) {
  return enqueue_command(cmd, say_ok, true);
}
...
void enqueue_and_echo_commands_now_P(const char * const pgcode) {
      enqueue_commands_now_P(pgcode, true);
    }

    void enqueue_commands_now_P(const char * const pgcode, bool echo=false) {
...
inline void gcode_M115() {
  ...
  cap_line(PSTR("THERMAL_PROTECTION")
          ...
          , advi3pp::Printer::is_thermal_protection_enabled()
#endif
  );
...
/**
  ...
  inline bool prepare_kinematic_move_to(const float (&rtarget)[XYZE]) {
    ...
    while (--segments) {
      ...
      #else
        if (!planner.buffer_line(delta[A_AXIS], delta[B_AXIS], raw[Z_AXIS], raw[E_AXIS], _feedrate_mm_s, active_extruder, cartesian_segment_mm))
          break;
      #endif
    }
