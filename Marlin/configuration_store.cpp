...
uint16_t MarlinSettings::datasize() { return sizeof(SettingsData) + 20; }
...
#define DUMMY_PID_VALUE 3000.0f
  ...
  #define EEPROM_ASSERT(TST,ERR,FIELD) if (!(TST)) {do{ SERIAL_ERROR_START(); SERIAL_ERRORLNPGM(ERR); }while(0);} else {do{SERIAL_ECHO_START(); SERIAL_ECHO("Field " STRINGIFY(FIELD)); SERIAL_ECHO(" "); SERIAL_ECHO(eeprom_index); SERIAL_ECHO(" "); SERIAL_ECHO(offsetof(SettingsData, FIELD)); SERIAL_ECHO("\n"); }while(0);}
